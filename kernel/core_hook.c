...
void escape_to_root(void)
{
    ...
    // setup capabilities
    // we need CAP_DAC_READ_SEARCH because `/data/adb/ksud` is not accessible for non root process
    // we add it here but don't add it to cap_inhertiable, it would be dropped automatically after exec!
...
static int nuke_ext4_sysfs(const char *custompath) {
    struct path path;
...
{
#ifdef CONFIG_KSU_SUSFS
    // Check if process is supposed to be unmounted to prevent side channel attacks
    ...
    // Find manager UID in throne tracker
    ...
    if (arg2 == CMD_NUKE_EXT4_SYSFS) {
        ...
        int ret = nuke_ext4_sysfs(buf);
        if (ret) {
            pr_err("cmd_nuke_ext4_sysfs: failed: %d\n", ret);
            return 0;
        }
    ...
    if (arg2 == CMD_TOGGLE_AVC_SPOOF) {
        ...
        if (arg3 == 0) {
            ...
            int ret = avc_spoof_init();
            if (ret) {
                pr_err("avc_spoof_init failed: %d\n", ret);
                return 0;
            }
    ...
    if (arg2 == CMD_REPORT_EVENT) {
        ...
        case EVENT_MODULE_MOUNTED: {
            if (!ksu_module_mounted) {
                ...
                int ret = nuke_ext4_sysfs("/data/adb/modules");
                if (ret) {
                    pr_err("nuke_ext4_sysfs failed: %d\n", ret);
                }
            }
            break;
        }
        default:
            pr_warn("Unknown event: %lu\n", arg3);
            break;
        }
        return 0;
    }
    ...
    if (arg2 == CMD_ENABLE_SU) {
        ...
        if (enabled) {
#ifdef CONFIG_KSU_SUSFS_SUS_SU
            // Disable all sus_su hooks when enabling su_kps
...
void susfs_try_umount_all(uid_t uid) {
    ...
    // /data/adb/modules is a loop device, so we can't determine if it's KSU, but it's safe to unmount
...
{
    // this hook is used for unmounting overlayfs for some uid, if there isn't any module mounted, just ignore it!
    if (!ksu_module_mounted) {
        return 0;
    }
    ...
    // ksu manager app uid is excluded in allow_list_arr, so ksu_uid_should_umount(manager_uid) will always return true
    if (ksu_is_manager_uid_valid() &&
        (new_uid.val % 1000000 == ksu_get_manager_uid())) {
        return 0;
    }

    // Check if spawned process is normal user app and needs to be unmounted
    if (likely(is_zygote_normal_app_uid(new_uid.val) && ksu_uid_should_umount(new_uid.val))) {
        goto do_umount;
    }

    // Lastly, check if spawned process is some system process and needs to be unmounted
...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
    // Reorder the mnt_id after all sus mounts are unmounted
...
int ksu_handle_setuid(struct cred *new, const struct cred *old)
{
    ...
    // fixme: use `collect_mounts` and `iterate_mount` to iterate all mountpoint and
    // filter-elle mountpoint whose target is `/data/adb`
    try_umount("/odm", true, 0);
    try_umount("/system", true, 0);
    try_umount("/vendor", true, 0);
    try_umount("/product", true, 0);
